function parsePayload(payload, params) {
    ...
    //ref https://github.com/ProVuUK/Alliot-decoders
    class syneticaEnLink {
        /**
         * Handle payload
         *
        ...
        static handle (payload) {
            ...
            const ENLINK_VIBRATION_EVT = 0x32;                      // U8 1 or 0, Leak status on resistance rope 1 or 0, vibration event detected

            const ENLINK_PRESSURE_TX = 0x3A;                        // U16 Pressure/Depth Transducer (0..50,000 mbar/mm)
            const ENLINK_TEMPERATURE_TX = 0x3B;                     // S16 Pressure/Depth Transducer (0..50,000 mbar/mm) Transducer Temperature - 3276.8°C -> 3276.7°C(-10..80)
            ...
            const ENLINK_NO2 = 0x54;                                // U16 0 ->> 655.35 ppm (0..5 ppm) [Divide by 100]
            const ENLINK_NO2_20 = 0x55;                             // U16 0 ->> 655.35 ppm (0..20 ppm) [Divide by 100]
            const ENLINK_SO2 = 0x56;                                // U16 0 ->> 655.35 ppm (0..20 ppm) [Divide by 100]
            ...
            const ENLINK_NC_PM0_5 = 0x5B;                           // F32 µg/m³ Number Concentration
            const ENLINK_NC_PM1_0 = 0x5C;                           // F32 µg/m³
            const ENLINK_NC_PM2_5 = 0x5D;                           // F32 µg/m³
            const ENLINK_NC_PM4_0 = 0x5E;                           // F32 µg/m³
            const ENLINK_NC_PM10_0 = 0x5F;                          // F32 µg/m³
                    ...
                    switch (data[i]) {
                        ...
                        case ENLINK_TEMP_PROBE_LOW_DURATION_S_1:
                            ...
                            obj.temp_probe_low_duration_s_1 =
                                U32((data[i + 1] << 24) | (data[i + 2] << 16) | (data[i + 3] << 8) | (data[i]));
                            i += 4;
                            msg_ok = true;
                            break;
            ...
            // payload should hopefully be a javascript array, so it needs parsing
            ...
            if (typeof payload === "object" && payload instanceof Array) {
                ...
                throw new PayloadError("The payload is not a valid JavaScript array.");
            }
        }

        /**
         * Check to see if string is valid JSON or not
         *
         * @param  {string} string
         * @return {boolean}
         */
    ...
    return syneticaEnLink.handle(payload);
}